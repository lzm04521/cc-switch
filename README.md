...
| 版本号 | `<x.y.z>` | `<上游版本>-exp.<N>`（如 `3.19.2-exp.1`） |

其余功能、发布版本、完整文档、多平台构建与上游一致，详见原仓库 [farion1231/cc-switch](https://github.com/farion1231/cc-switch)。
...
- 从上游正式版切换到本 Fork 需手动安装一次：版本号语义上 `-exp.N` 低于同号正式版，不会被当作自动升级推送。
...
- `local/v<上游版本>-exp.<N>`：发版分支，改版本号、打 tag、推送后由 [`release.yml`](.github/workflows/release.yml) 自动构建发布。
