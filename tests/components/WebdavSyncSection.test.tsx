...
describe("WebdavSyncSection", () => {
  ...
  it("saves startup delay minutes as a number", async () => {
    renderSection({ ...baseConfig, startupDelayMinutes: 5 });

    const input = screen.getByRole("spinbutton", {
      name: "settings.webdavSync.startupDelay",
    }) as HTMLInputElement;
    ...
    await waitFor(() => {
      expect(settingsApiMock.webdavSyncSaveSettings).toHaveBeenCalledWith(
        expect.objectContaining({ startupDelayMinutes: 10 }),
        false,
      );
    });
  });
