...
use providers::{claude, codex, gemini, grokbuild, hermes, openclaw, opencode, zcode};
...
pub fn scan_sessions() -> Vec<SessionMeta> {
    let (r1, r2, r3, r4, r5, r6, r7, r8) = std::thread::scope(|s| {
        ...
        let h8 = s.spawn(zcode::scan_sessions);
        (
...
pub fn load_messages(provider_id: &str, source_path: &str) -> Result<Vec<SessionMessage>, String> {
    ...
    if provider_id == "zcode" && source_path.starts_with("sqlite:") {
        return zcode::load_messages_sqlite(source_path);
    }
    ...
    match provider_id {
        ...
        // zcode 只有 SQLite 源（无文件版消息目录），上面已处理
...
pub fn delete_session(
    ...
    if provider_id == "zcode" && source_path.starts_with("sqlite:") {
        return zcode::delete_session_sqlite(session_id, source_path);
    }
