...
pub fn start_worker(db: Arc<crate::database::Database>, app: tauri::AppHandle) {
    ...
    let startup_delay_minutes = settings::get_s3_sync_settings()
...
async fn run_worker_loop(
    ...
    if startup_delay_minutes > 0 {
        ...
        if had_change {
            log::info!(
                "[S3][AutoSync] Startup delay {startup_delay_minutes}min elapsed, flushing pending backup"
            );
            if let Err(err) = run_auto_sync_upload(&db, &app).await {
                log::warn!("[S3][AutoSync] Startup-delay flush failed: {err}");
            }
        }
    }
