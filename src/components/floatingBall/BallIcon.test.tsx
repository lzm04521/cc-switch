...
describe("BallIcon", () => {
  ...
  it("移动超过阈值交给后端原生循环拖动（只触发一次），松手不触发 togglePanel", () => {
    ...
    fireEvent.pointerMove(el, { pointerId: 1, screenX: 16, screenY: 12 });
    expect(startDrag).toHaveBeenCalledTimes(1);
    expect(startDragging).not.toHaveBeenCalled();
    // 后续 move 不再重复触发（前端已退出本次手势）
    fireEvent.pointerMove(el, { pointerId: 1, screenX: 100, screenY: 100 });
    expect(startDrag).toHaveBeenCalledTimes(1);
  ...
  it("位移未达拖动阈值（且未达点击阈值）的移动不进入拖动，松手仍为点击", () => {
    ...
    fireEvent.pointerMove(el, { pointerId: 1, screenX: 13, screenY: 12 });
